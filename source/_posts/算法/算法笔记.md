...
int main()
{
	ios::sync_with_stdio(0),cin.tie(0),cout.tie(0);//不能使用scanf
    int t;cin >> t;
    while(t--)
    {
      solve();
...
3. n ≤ 1000, 时间复杂度 O(n²)，O(n²logn):
   - 算法: DP，二分，朴素版Dijkstra、朴素版Prim、Bellman-Ford

4. n ≤ 10000, 时间复杂度 O(n * √n):
   - 算法: 块状链表、分块、莫队

5. n ≤ 100000, 时间复杂度 O(nlogn):
   - 算法: 各种sort，线段树、树状数组、set/map、heap、拓扑排序、Dijkstra + heap、Prim + heap、Kruskal、SPFA、求凸包、求半平面交、二分、CDQ分治、整体二分、后缀数组、树链剖分、动态树

6. n ≤ 1000000, 时间复杂度 O(n), 以及常数较小的 O(nlogn)算法:
   - 算法: 单调队列、hash、双指针扫描、BFS、并查集、KMP、AC自动机、常数比较小的 O(nlogn)的做法：sort、树状数组、heap、Dijkstra、SPFA

7. n ≤ 10000000, 时间复杂度 O(n):
   - 算法: 双指针扫描、KMP、AC自动机、线性筛素数

8. n ≤ 10^9, 时间复杂度 O(√n):
   - 算法: 判断质数

9. n ≤ 10^18, 时间复杂度 O(logn):
   - 算法: 最大公约数，快速幂，数位DP

10. n ≤ 10^1000, 时间复杂度 O((logn)²):
    - 算法: 高精度加减乘除

11. n ≤ 10^10000, 时间复杂度 O(logk × loglogk):
    - 算法: k表示位数，高精度加减、FFT/NTT
